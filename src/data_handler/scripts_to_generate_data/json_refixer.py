...
from src.utils.azure_client import get_embedding, get_answer
import src.utils.answer_extraction as answer_extraction
...
def extract_list(data):
    ...
    extracted_list = answer_extraction.extract_list(data["list"])
...
def add_original_embeddings(data):
    return get_embedding(data["scenario"], model="text-embedding-3-small")

def add_answer(data):
    return get_answer(data["scenario"])


def add_answer_embeddings(data):
    return get_embedding(data["original_answer"], model="text-embedding-3-small")
